...
class Filipad2 {
    ...
    public:
        ...
        Filipad2(int inID=1, float inRelSize=1.1, float inR = 0.4, float inR2 = 0.2, int inXOffset = 100, int inYOffset=100, float inAspect=0.7, int ichop=5, int ichopPt=3){
            aspektCanvas = inAspect;
            sizeCanvas   = 300*inRelSize;
        ...
        void Draw(){ 
            ...
            if (ratio2) {
                pp = new TPad(Form("LPad%d",ID),Form("LPad%d",ID),0, ratio2, 1, ratio ,0); //create pad
                pp->SetNumber(2);   //assign a number to it. Possible to access it via :  toppad->cd(ih); 
                pp->SetTopMargin(0.0015); pp->SetBottomMargin(0.0015);//our pads will have no margin
                pp->SetLeftMargin(MarginLeft);pp->SetRightMargin(MarginRight);
                pp->Draw();

                pp = new TPad(Form("LPad%d",ID),Form("LPad%d",ID),0, 0, 1, ratio2 ,0); //create pad
                pp->SetNumber(3);   //assign a number to it. Possible to access it via :  toppad->cd(ih); 
                pp->SetTopMargin(0.0015); pp->SetBottomMargin(MarginBottom/ratio2);//our pads will have no margin
                pp->SetLeftMargin(MarginLeft);pp->SetRightMargin(MarginRight);
                pp->Draw();
            } else {
