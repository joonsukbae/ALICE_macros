...
TString mainDir = "../../jets/mc/trackefficiency/AnalysisResults/";
std::vector<TString> fileNames = {
    ...
    // "../../jetfinderQA/AnalysisResults/LHC24b1b/AnalysisResults_hy.root",
    // "../../jetfinderQA/AnalysisResults/LHC23d1k/AnalysisResults_hy_MatchColl_trkeff_trkres.root"
    // "LHC23d1k/AnalysisResults_HY_global_uniform_dcaZmax_2.root"
    // "../../jetfinderQA/AnalysisResults/LHC23d1k/AnalysisResults_hy_MatchColl_trkeff_trkres.root"
    "LHC24f3/selMC/AnalysisResults.root"
    };
// std::vector<TString> histNames = {"LHC24b1b_uniformTracks"};
std::vector<TString> histNames = {"LHC24f3"};
...
void DrawTrackResolution() { DrawHistos(fileNames, histNames, ColorPalette); }
...
TH1 *DrawTrackResolution(const char *fileName, const char *histName,
  ...
  TH2 *TrackResolutionTemp1 = (TH2 *)file->Get(Form("%s/%s", Dir, TrackResolutionObj));
  TH2 *TrackResolutionTemp2 = (TH2 *)file->Get(Form("%s/%s", Dir, TrackResolutionObj2));
  ...
  for (Int_t i = 0; i < pTClasses.size(); i++) {
    ...
    hoptset(*trackResolution, 2., ColorPalette[i], -1., 1., 1e-9, 1e0);
...
void DrawHistos(const std::vector<TString> &fileNames,
  ...
  for (Int_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
    ...
    TCanvas *canTrackResolution =
        new TCanvas(Form("TrackResolution_%s", histNames[i].Data()),
                    Form("trackresolution%s", histNames[i].Data()), 800, 600);
    ...
    DrawTrackResolution(filePath.Data(), histNames[i].Data(),
                        legtrackresolution, legtrackresolutionStat,
                        ColorPalette[i]);
    legtrackresolution->Draw();
    legtrackresolutionStat->Draw();
    canTrackResolution->Print(
        Form("plots/AN_Charged-particle-jet-cross-section-in-pp-collisions-at-13.6-TeV/Figures/trackQA/TrackResolution_R%.1f_%s.pdf", RBIN, histNames[i].Data()));
  }
}