...
// Double_t RBIN = 0.6;
Int_t Npthat = 2;
const char *Dir = "jet-finder-qa";
const char *EvtDir = "event-selection-task";
const char *EventObj = "hColCounterAcc";
...
std::vector<TString> fileNames = {"AnalysisResults_LHC23d4_HY_unweighted.root"};
std::vector<TString> histNames = {"MC Anchored", "Jet-MC"};
...
std::vector<TH1F*> DrawJetMatching(const char* fileName, const char* DfileName, const char* histName, float Nevts, TPad* jrep, TPad* ratiojrep, TLegend* JRElegend, TPad* consistencyp, TPad* ratioconsistp, TLegend* CONSISTlegend, TPad* unfoldp, TPad* ratunfoldp, TLegend* unfoldlegend, Color_t colorID) {
    ...
    // cout<<"Nbins JetMCDPt: "<<JetMCDPt->GetNbinsX()<<endl;
    TH3F *HCorrelate = (TH3F *) gROOT->FindObject("h3_jet_r_jet_pt_part_jet_pt");
    ...
    hoptset(*JREp, Nevts, kBlack, 0, 200, 1e-9, 1e0);
    ...
    hoptset(*mcpmatchedpt, Nevts, kRed, 0, 200, 1e-12, 1e0);
    ...
    hoptset(*UnfoldMC, Nevts, kRed, 0, 200, 1e-9, 1e0);
    ...
    hoptset(*UnfoldData, Nevts, kRed, 0, 200, 1e-9, 1e0);
...
void DrawHistos(const std::vector<TString>& fileNames, const std::vector<TString>& histNames, const std::vector<int>& ColorPallete) {
    ...
    if(TrackProcess==1) {
        ...
        trackptpad->cd();
        // TH1* TrackPtRatio = DrawTrackPt(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legtrackpt, kBlack);
        ...
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            ...
            // TH1* currentHist = DrawTrackPt(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legtrackpt, ColorPallete[i]);
        ...
        tracketapad->cd();
        // TH1* TrackEtaRatio = DrawTrackEta(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legtracketa, kBlack);
        TH1* TrackEtaRatio = DrawTrackEta(refPath.Data(), refName, 1., legtracketa, kBlack);
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            tracketapad->cd(); 
            TString filePath = mainDir + fileNames[i];
            // TH1* currentHist = DrawTrackEta(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legtracketa, ColorPallete[i]);
        ...
        trackphipad->cd();
        // TH1* TrackPhiRatio = DrawTrackPhi(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legtrackphi, kBlack);
        ...
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            trackphipad->cd(); 
            TString filePath = mainDir + fileNames[i];
            // TH1* currentHist = DrawTrackPhi(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legtrackphi, ColorPallete[i]);
...
cout<<Form("Debugger_%i",n++)<<endl;
    if (ConstituentProcess==1) {
        ...
        // constptpad->cd();
        // TH1* ConstPtRatio = DrawConstituentPt(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legconstpt, kBlack);
cout<<Form("Debugger_%i",n++)<<endl;        
...
cout<<Form("Debugger_%i",n++)<<endl;        
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            constptpad->cd(); 
            TString filePath = mainDir + fileNames[i];
            // TH1* currentHist = DrawConstituentPt(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legconstpt, ColorPallete[i]);
...
cout<<Form("Debugger_%i",n++)<<endl;        
        ...
        constetapad->cd();
        // TH1* ConstEtaRatio = DrawConstituentEta(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legconsteta, kBlack);
        TH1* ConstEtaRatio = DrawConstituentEta(refPath.Data(), refName, 1, legconsteta, kBlack);
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            constetapad->cd(); 
            TString filePath = mainDir + fileNames[i];
            // TH1* currentHist = DrawConstituentEta(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legconsteta, ColorPallete[i]);
        ...
        constphipad->cd();
        // TH1* ConstPhiRatio = DrawConstituentPhi(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legconstphi, kBlack);
        TH1* ConstPhiRatio = DrawConstituentPhi(refPath.Data(), refName, 1, legconstphi, kBlack);
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            constphipad->cd(); 
            TString filePath = mainDir + fileNames[i];
            // TH1* currentHist = DrawConstituentPhi(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legconstphi, ColorPallete[i]);
...
cout<<Form("Debugger_%i",n++)<<endl; 
    if(JetProcess==1) {
        ...
        jetptpad->cd();
        TH1* JetPtRatio = DrawJetPt(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legjetpt, kBlack);
        // TH1* JetPtRatio = DrawJetPt(refPath.Data(), refName, 1., legjetpt, kBlack);
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            jetptpad->cd(); 
            TString filePath = mainDir + fileNames[i];
            TH1* currentHist = DrawJetPt(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legjetpt, ColorPallete[i]);
        ...
        jetetapad->cd();
        // TH1* JetEtaRatio = DrawJetEta(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legjeteta, kBlack);
        TH1* JetEtaRatio = DrawJetEta(refPath.Data(), refName, 1., legjeteta, kBlack);
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            jetetapad->cd(); 
            TString filePath = mainDir + fileNames[i];
            // TH1* currentHist = DrawJetEta(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legjeteta, ColorPallete[i]);
        ...
        jetphipad->cd();
        // TH1* JetPhiRatio = DrawJetPhi(refPath.Data(), refName, Nevents(refPath.Data(),EvtDir,EventObj), legjetphi, kBlack);
        TH1* JetPhiRatio = DrawJetPhi(refPath.Data(), refName, 1., legjetphi, kBlack);
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            jetphipad->cd(); 
            TString filePath = mainDir + fileNames[i];
            // TH1* currentHist = DrawJetPhi(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legjetphi, ColorPallete[i]);
        ...
        //     TString filePath = mainDir + fileNames[i];
        //     // TH1* currentHist = DrawJetArea(filePath.Data(), histNames[i].Data(), Nevents(filePath.Data(),EvtDir,EventObj), legjetarea, ColorPallete[i]);
    ...
    cout<<Form("Debugger_BF JETmatching",n++)<<endl;
    if (JetMatchingProcess==1) {
        for (size_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            ...
            TString filePath = mainDir + fileNames[i];
            DrawJetMatching(filePath.Data(), refPath.Data(), histNames[i].Data(), 1, jrepad, ratiojrepads, legjre, consistpad, ratioconsistpad, legconsist, unfoldpad, ratiounfoldpad, legunfold, ColorPallete[i]);
