...
auto JetProcess = 1;
auto JetMatchingProcess = 0;
// auto Normalization = 1; // (1: probability dist, 2: Number of events)
...
void hoptset(TH1& hid, Float_t N = 1, Color_t color = kBlack, Double_t minX = 0, Double_t maxX = 100, Double_t minY = 0, Double_t maxY = 1, Double_t MarkerSize = 1, Double_t MarkerStyle = 0) {
...
// Run 2 objects from Yongzhen Hou
TString Run2File = "../../jets/Run2_YongzhenHou/FinalsixGeneratorPP13TeV.root";
const char *Run2JetPtObj = "Jetptetaphi04";
const char *Run2JetPtGenObj = "GenJetptetaphi04";
...
// std::vector<TString> fileNames = {"AnalysisResults_LHC23d1k_HY_3.root", "AnalysisResults_LHC23d4_HY_4.root",  "AnalysisResults_LHC23d4_HY_4_N_3.root"};
// std::vector<TString> fileNames = {"AnalysisResults_LHC23d1k_HY_27Jan2024_ITS1.root", "AnalysisResults_LHC23d4_HY_27Jan2024_ITS1.root"};
std::vector<TString> fileNames = {"AnalysisResults_LHC23d4_HY_27Jan2024_ITS1.root"};
// std::vector<TString> histNames = {"MC: LHC22d1k (anchored)", "MC: LHC23d4 (jet MC, N=2)", "MC: LHC23d4 (jet MC, N=3)"};
std::vector<TString> histNames = {"Jet-MC"};
...
const char* JetResolutionObjITS = "h3_jet_r_jet_pt_tag_jet_pt_base_diff_matchedgeo";
...
const char* JetPtGenTitleX = "#it{p}_{T, jet}^{true} (GeV/c)";
...
TH1* DrawRatio(const char* ratioName, TH1* refHist, TH1* testHist, TString AxisTitleX, TString AxisTitleY, Color_t colorID, Double_t Ymin = 0, Double_t Ymax = 1.5, Double_t MarkerSize = 1, Double_t MarkerStyle = 0) {
...
// Draw Jets
TH1* DrawJetPt(const char* fileName, const char* histName, float Nevts, TLegend* legend, Color_t colorID, Int_t i = 0) {
    auto file = TFile::Open(fileName, "open");
    auto dir = (TDirectory*) file->Get(Dir);
    dir->cd();
    ...
    hoptset(*JetPt, Nevts, colorID, 0, 200, 1e-8, 1e0, 1);
    JetPt->Draw("esame");
...
TH1* DrawJetPtMCP(const char* fileName, const char* histName, float Nevts, TLegend* legend, Color_t colorID, Int_t i = 0) {
    ...
    hoptset(*JetPtMCP, Nevts, colorID, 0, 200, 1e-8, 1e0, 1.2 - 0.1*i);
    JetPtMCP->Draw("esame");
...
TH1* DrawJetArea(const char* fileName, const char* histName, float Nevts, TLegend* legend, Color_t colorID) {
    ...
    // hoptset(*JetArea, Nevts, colorID, 0, 200, 0., 3.);
    JetArea->Scale(1./JetArea->Integral(), "width");
    // JetArea->Scale(1./Nevts, "width");
...
std::vector<TH1F*> DrawJetMatching(const char* fileName, const char* DfileName, const char* histName, float Nevts, TPad* jrep, TPad* ratiojrep, TLegend* JRElegend, TPad* consistencyp, TPad* ratioconsistp, TLegend* CONSISTlegend, TPad* unfoldp, TPad* ratunfoldp, TLegend* unfoldlegend, Color_t colorID, TPad* jrpp = nullptr, TPad* ratiojrpp = nullptr, TLegend* JRPlegend = nullptr) {
    ...
    // TH1 *MCDMatchedpt = (TH1 *) HCorrelate->ProjectionZ(Form("hMCDMatched_%s", histName), HCorrelate->GetXaxis()->FindBin(RBIN + 1e-6), HCorrelate->GetXaxis()->FindBin(RBIN + 0.2 - 1e-6), 1, HCorrelate->GetNbinsY(), "e");
    // MCDMatchedpt = MCDMatchedpt->Rebin(nptBins, Form("MCDptMatched_%s", histName), ptbin);
    // TH1 *MCPMatchedpt = (TH1 *) HCorrelate->ProjectionY(Form("hMCPMatched_%s", histName), HCorrelate->GetXaxis()->FindBin(RBIN + 1e-6), HCorrelate->GetXaxis()->FindBin(RBIN + 0.2 - 1e-6), 1, HCorrelate->GetNbinsX(), "e");
    ...
    TCanvas* canCorrelation = new TCanvas(Form("Correlation_%s", histName), Form("c%s", histName), 800, 800);
    canCorrelation->cd();
    ...
    canCorrelation->Print(Form("plots/2024Jan_KoALICE/JetPtCorrelation_R%.1f_%s.pdf", RBIN, histName));
...
TH1* DrawJetResolution(const char* fileName, const char* histName, TLegend* legend, TLegend* legend2, Color_t colorID) {
    ...
    TH3 *JetResolution;
    if (histNameStr.Contains("ITS")) {
      JetResolution = (TH3 *) gROOT->FindObject(JetResolutionObjITS);
    } else {
      JetResolution = (TH3 *) gROOT->FindObject(JetResolutionObj);
    }
...
void DrawHistos(const std::vector<TString>& fileNames, const std::vector<TString>& histNames, const std::vector<int>& ColorPallete) {
    // Run 2 analysis
    auto Run2file = TFile::Open(Run2File, "open");
    auto *Run2Nevt = (TH1 *) gROOT->FindObject("fHistZVertex");
    auto run2nevt = Run2Nevt->Integral(Run2Nevt->GetXaxis()->FindBin(-10),Run2Nevt->GetXaxis()->FindBin(10));
    // auto run2nevt = 1; 
    auto *H3JetPt = (TH3 *) gROOT->FindObject(Run2JetPtObj);
    TH1 *run2jetpt = H3JetPt->ProjectionZ("Run2JetPt", H3JetPt->GetXaxis()->FindBin(-0.5), H3JetPt->GetXaxis()->FindBin(0.5), H3JetPt->GetYaxis()->FindBin(.0), H3JetPt->GetYaxis()->FindBin(2*TMath::Pi()));
    // run2jetpt->GetXaxis()->SetRangeUser(5,200);
    run2jetpt = run2jetpt -> Rebin(nptBins,"run2jetpt",ptbin);
    auto *H3JetPtGen = (TH3 *) gROOT->FindObject(Run2JetPtGenObj);
    TH1 *run2jetptgen = H3JetPtGen->ProjectionZ("Run2JetGenPt", H3JetPtGen->GetXaxis()->FindBin(-0.5), H3JetPtGen->GetXaxis()->FindBin(0.5), H3JetPtGen->GetYaxis()->FindBin(.0), H3JetPtGen->GetYaxis()->FindBin(2*TMath::Pi()));
    run2jetptgen = run2jetptgen -> Rebin(nptBins,"run2jetptgen",ptbin); 
    ...
    if(TrackProcess==1) {
        ...
        trackptpad->cd();
        legtrackpt->Draw();
        TrackPtPad->C->Print(Form("plots/2024Jan_KoALICE/TrackPt_ITS%i.pdf", selITS));
        ...
        tracketapad->cd();
        legtracketa->Draw();
        TrackEtaPad->C->Print(Form("plots/2024Jan_KoALICE/TrackEta_ITS%i.pdf", selITS));
        ...
        trackphipad->cd();
        legtrackphi->Draw();
        TrackPhiPad->C->Print(Form("plots/2024Jan_KoALICE/TrackPhi_ITS%i.pdf", selITS));
    }
    ...
    if (ConstituentProcess==1) {
        ...
        constptpad->cd();
        legconstpt->Draw();
        ConstPtPad->C->Print(Form("plots/2024Jan_KoALICE/ConstPt_R%.1f_ITS%i.pdf",RBIN,selITS));
        ...
        ConstEtaPad->C->Print(Form("plots/2024Jan_KoALICE/ConstEta_R%.1f_ITS%i.pdf",RBIN,selITS));
        ...
        ConstPhiPad->C->Print(Form("plots/2024Jan_KoALICE/ConstPhi_R%.1f_ITS%i.pdf",RBIN,selITS));
    }
    ...
    if(JetProcess==1) {
        ...
        jetptpad->cd();
        hoptset(*run2jetpt, run2nevt, kBlack, 5, 200, 1e-8, 1e0, 1.2, 34);
        legjetpt->AddEntry(run2jetpt, "Run 2");
        ...
        for (Int_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            ...
            // TH1* currentHist = DrawJetPt(filePath.Data(), histNames[i].Data(), 1, legjetpt, ColorPallete[i], i);
            TString ratioName = TString::Format("RatioHist_jpt_%s", fileNames[i].Data());
            ratiojetptpad->cd();
            TH1* ratioHist = DrawRatio(ratioName.Data(), run2jetpt, currentHist, JetPtTitleX, "Run 3 / Run 2", ColorPallete[i], 0., 3.5);
        }

        jetptpad->cd();
        run2jetpt->Draw("epsame");
        legjetpt->Draw();
        JetPtPad->C->Print(Form("plots/2024Jan_KoALICE/JetPt_R%.1f_ITS%i.pdf",RBIN,selITS));
        ...
        hoptset(*run2jetptgen, run2nevt, kBlack, 5, 200, 1e-8, 1e1, 1.2, 34);
        legjetptmcp->AddEntry(run2jetptgen, "Run 2");
        // TString JetPtMCPfilePath = mainDir + fileNames[0];
        // TH1* JetPtMCPRatio = DrawJetPtMCP(JetPtMCPfilePath.Data(), histNames[0].Data(), Nevents(JetPtMCPfilePath.Data(),Dir,EventObj), legjetptmcp, kBlack);
        // TH1* JetPtMCPRatio = DrawJetPtMCP(JetPtMCPfilePath.Data(), histNames[0].Data(), 1, legjetptmcp, kBlack);
        for (Int_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            jetptmcppad->cd(); 
            TString filePath = mainDir + fileNames[i];
            ...
            ratiojetptmcppad->cd();
            TH1* ratioHist = DrawRatio(ratioName.Data(), run2jetptgen, currentHist, JetPtGenTitleX, "Run 3 / Run 2", ColorPallete[i], 0.5, 2.5, 1);
        }
        jetptmcppad->cd();
        run2jetptgen->Draw("epsame");
        legjetptmcp->Draw();
        JetPtMCPPad->C->Print(Form("plots/2024Jan_KoALICE/JetPtMCP_R%.1f_ITS%i.pdf",RBIN,selITS));
    ...
    // cout<<Form("Debugger_%i",n++)<<endl;
        ...
        JetEtaPad->C->Print(Form("plots/2024Jan_KoALICE/JetEta_R%.1f_ITS%i.pdf",RBIN,selITS));
        ...
        JetPhiPad->C->Print(Form("plots/2024Jan_KoALICE/JetPhi_R%.1f_ITS%i.pdf",RBIN,selITS));
        ...
        jetntrackspad->cd();
        legjetntracks->Draw();
        JetNtracksPad->C->Print(Form("plots/2024Jan_KoALICE/JetNtracks_R%.1f_ITS%i.pdf",RBIN,selITS));
        ...
        for (Int_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            ...
            legjetarea->Draw();
            canJetArea->Print(Form("plots/2024Jan_KoALICE/JetArea_R%.1f_%s_ITS%i.pdf",RBIN, histNames[i].Data(), selITS));
        }
    }
    ...
    if (JetMatchingProcess==1) {
        for (Int_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            ...
            TString filePath = mainDir + fileNames[i];
            DrawJetMatching(filePath.Data(), refPath.Data(), histNames[i].Data(), Nevents(filePath.Data(),Dir,EventObj), jrepad, ratiojrepads, legjre, consistpad, ratioconsistpad, legconsist, unfoldpad, ratiounfoldpad, legunfold, ColorPallete[i], jrppad, ratiojrppads, legjrp);
            ...
            jrppad->cd();
            legjrp->Draw();
            JRPPad->C->Print(Form("plots/2024Jan_KoALICE/JRP_R%.1f_%s_ITS%i.pdf", RBIN, histNames[i].Data(), selITS));

            consistpad->cd();
            legconsist->Draw();
            ConsistencyPad->C->Print(Form("plots/2024Jan_KoALICE/ConsistencyCheck_R%.1f_%s_ITS%i.pdf", RBIN, histNames[i].Data(), selITS)); 

            unfoldpad->cd();
            legpt2->Draw();
            legpt3->Draw();
            legunfold->Draw();
            UnfoldPad->C->Print(Form("plots/2024Jan_KoALICE/UnfoldedJetPt_R%.1f_%s_ITS%i.pdf", RBIN, histNames[i].Data(), selITS)); 
        }
        ...
        for (Int_t i = 0; i < std::min(fileNames.size(), histNames.size()); ++i) {
            ...
            canJetResolution->Print(Form("plots/2024Jan_KoALICE/JetResolution_R%.1f_%s_ITS%i.pdf",RBIN, histNames[i].Data(), selITS));
        }
